...
import { useAjustesDesayuno, type AjustesDesayuno } from "@/lib/ajustes-desayuno-context";
...
type FormPct = Record<keyof AjustesDesayuno, string>;

function aFormPct(a: AjustesDesayuno): FormPct {
  return {
...
const CAMPOS: Array<{ clave: keyof AjustesDesayuno; label: string; desc: string }> = [
  {
...
export default function DesayunosAjustesPage() {
  const { recargar } = useAjustesDesayuno();
  ...
  async function guardar() {
    ...
    setGuardando(true);
    try {
      const cambios: Partial<AjustesDesayuno> = {};
  ...
  return (
    <DashboardShell title="Ajustes" subtitle="Desayunos · objetivos y umbrales de alerta">
      <div className="p-6 space-y-6 max-w-[700px] mx-auto">
        ...
        <section className="rounded-xl border border-border bg-surface p-5 shadow-soft space-y-5">
