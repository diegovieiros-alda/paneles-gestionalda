...
export const Route = createFileRoute("/oportunidades")({
  head: () => ({
    meta: [
      { title: "Oportunidades · Breakfast OS" },
      { name: "description", content: "Hoteles ordenados por oportunidad de facturación no capturada y acción sugerida." },
    ],
  }),
              ...
              {rows.slice(0, 40).map(({ h, o, accion }, i) => (
                <tr key={h.id} className="border-t border-border hover:bg-accent/30 transition-colors group">
                  <td className="px-4 py-3">
                    <Link to="/hoteles/$hotelId" params={{ hotelId: h.id }} className="font-medium text-foreground hover:text-primary">
                  ...
                  <td className="pr-3">
                    <Link to="/hoteles/$hotelId" params={{ hotelId: h.id }} className="text-muted-foreground hover:text-primary inline-flex">
